...
public class ListController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> getListItem(@PathVariable String id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateListItem(@PathVariable String id, @RequestBody Map<String, String> payload) {
        Optional<ListItem> itemOpt = repository.findById(id);
        if (!itemOpt.isPresent()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteListItem(@PathVariable String id) {
        Optional<ListItem> itemOpt = repository.findById(id);
        if (!itemOpt.isPresent()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
